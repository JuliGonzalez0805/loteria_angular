...
/**
 * Componente de Solicitud de Visita para Visitantes
 * Permite a un visitante solicitar una visita sin autenticación
 */
@Component({
    ...
    <div class="solicitud-container">
      ...
      <header class="solicitud-header">
        ...
        <div class="header-content">
          <h1 class="header-title">Solicitud de Visita</h1>
          <p class="header-subtitle">Complete el formulario para solicitar su visita</p>
        </div>
      </header>
            ...
            <div class="form-section">
              ...
              <div class="form-row">
                <mat-form-field appearance="outline" class="form-field">
                  ...
                  <input matInput formControlName="nombre" placeholder="Su nombre">
                  <mat-icon matPrefix>person</mat-icon>
                  @if (solicitudForm.get('nombre')?.hasError('required') && solicitudForm.get('nombre')?.touched) {
                ...
                <mat-form-field appearance="outline" class="form-field">
                  ...
                  <input matInput formControlName="apellido" placeholder="Su apellido">
                  <mat-icon matPrefix>person</mat-icon>
                  @if (solicitudForm.get('apellido')?.hasError('required') && solicitudForm.get('apellido')?.touched) {
              ...
              <div class="form-row">
                <mat-form-field appearance="outline" class="form-field">
                  ...
                  @if (solicitudForm.get('dni')?.hasError('required') && solicitudForm.get('dni')?.touched) {
                    <mat-error>El DNI es requerido</mat-error>
                  }
                  @if (solicitudForm.get('dni')?.hasError('pattern')) {
                ...
                <mat-form-field appearance="outline" class="form-field">
                  ...
                  @if (solicitudForm.get('email')?.hasError('required') && solicitudForm.get('email')?.touched) {
                    <mat-error>El email es requerido</mat-error>
                  }
                  @if (solicitudForm.get('email')?.hasError('email')) {
            ...
            <div class="form-section">
              ...
              <div class="form-row">
                <mat-form-field appearance="outline" class="form-field">
                  ...
                  @if (solicitudForm.get('personaAVisitar')?.hasError('required') && solicitudForm.get('personaAVisitar')?.touched) {
                ...
                <mat-form-field appearance="outline" class="form-field">
                  ...
                  <mat-select formControlName="area" placeholder="Seleccione el área" (selectionChange)="onAreaChange()">
                    @for (area of areasDisponibles; track area.value) {
                      <mat-option [value]="area.value">{{ area.label }}</mat-option>
                    }
                  </mat-select>
                  <mat-icon matPrefix>business</mat-icon>
                  @if (solicitudForm.get('area')?.hasError('required') && solicitudForm.get('area')?.touched) {
              ...
              <div class="form-row">
                <mat-form-field appearance="outline" class="form-field">
                  ...
                  @if (solicitudForm.get('fecha')?.hasError('required') && solicitudForm.get('fecha')?.touched) {
                ...
                <mat-form-field appearance="outline" class="form-field">
                  ...
                  @if (solicitudForm.get('horario')?.hasError('required') && solicitudForm.get('horario')?.touched) {
  ...
  styles: [`
    .solicitud-container {
      @apply min-h-screen flex flex-col bg-gradient-to-br from-green-50 to-gray-100;
    }
...
export class SolicitudVisitaComponent implements OnInit {
  ...
  readonly horariosDisponibles = signal<Array<{value: string, label: string}>>([]);
  ...
  // Horarios base (se actualizarán según el área)
  private readonly allHorarios = [
  ...
  ngOnInit(): void {
    ...
    this.filteredPersonas$ = this.solicitudForm.get('personaAVisitar')!.valueChanges.pipe(
      startWith(''),
      map(value => this._filterPersonas(value || ''))
    );

    // Inicializar horarios
    this.horariosDisponibles.set(this.allHorarios);
  }
  ...
  // Filtro de fechas disponibles (ejemplo: solo días hábiles)
  ...
  onAreaChange(): void {
    // Actualizar horarios disponibles según el área seleccionada
    const area = this.solicitudForm.get('area')?.value;
    
    // TODO: Consultar al backend los horarios disponibles para el área
    // Por ahora, simulamos diferentes horarios según el área
    if (area === 'sistemas') {
      // Sistemas tiene horarios más limitados
      this.horariosDisponibles.set(this.allHorarios.slice(2, 8));
    } else if (area === 'comercial') {
      // Comercial tiene todos los horarios
      this.horariosDisponibles.set(this.allHorarios);
    } else {
      // Otros tienen horarios estándar
      this.horariosDisponibles.set(this.allHorarios.slice(1, 9));
    }

    // Limpiar selección de horario si ya no está disponible
    const currentHorario = this.solicitudForm.get('horario')?.value;
    const isStillAvailable = this.horariosDisponibles().some(h => h.value === currentHorario);
    if (!isStillAvailable) {
      this.solicitudForm.patchValue({ horario: '' });
    }
  }
  ...
  onSubmit(): void {
    ...
    // TODO: Integrar con el servicio backend
    console.log('Solicitud de visita:', formValue);

    // Simulación de envío
    setTimeout(() => {
      this.isLoading.set(false);
      this.successMessage.set('Solicitud enviada exitosamente. Recibirá una notificación por email.');
